...
class GhostModule(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, dw_size=3, ratio=2, stride=1,
        ...
        self.conv2 = nn.Conv2d(self.init_channels, self.new_channels, self.dw_size, 1, 
                               padding=int(self.dw_size/2), groups=self.init_channels)
...
def conv3x3(in_planes, out_planes, stride=1, s=4, d=3):
    return GhostModule(in_planes, out_planes, kernel_size=3, dw_size=d, ratio=s, stride=stride, padding=1, bias=False)
...
class Ghost_ResNet(nn.Module):
    def __init__(self, block = Bottleneck, layers = [3, 4, 6, 3], num_classes=3, s=4, d=3):
        self.inplanes = 64
        super(Ghost_ResNet, self).__init__()
