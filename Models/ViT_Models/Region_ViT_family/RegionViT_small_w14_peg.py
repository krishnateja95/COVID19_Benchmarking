...
__all__ = ['RegionViT_Small_w14_peg']
    ...
    def __init__(self, input_channels, output_channels, act_layer, mode='sc'):
        ...
        if 'c' in mode:
            ...
            if input_channels == output_channels and ks == 1:
                ...
                tmp.append(nn.Conv2d(in_channels=input_channels, out_channels=output_channels, kernel_size=ks,
    ...
    def __init__(self, input_channels, output_channels, kernel_size, num_blocks, num_heads, mlp_ratio=1., 
        ...
        for i in range(num_blocks):
            ...
            tmp.append(R2LAttentionPlusFFN(output_channels, output_channels, kernel_size_, num_heads, mlp_ratio, qkv_bias, qk_scale,
                                           ...
                                           attn_drop=attn_drop_rate, drop=drop_rate, cls_attn=cls_attn))
...
class RegionViT_Small_w14_peg(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=3, embed_dim=[96, 96, 192, 384, 768], depth=[2, 2, 8, 2],
                 num_heads=[3, 6, 12, 24], mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 kernel_sizes=[14, 14, 14, 14], downsampling=['c', 'sc', 'sc', 'sc'],
                 patch_conv_type='3conv',
                 computed_cls_token=True, peg=True,
                 det_norm=False):
        super().__init__()
