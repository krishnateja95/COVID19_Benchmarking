...
class DeepViT_16(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=3, embed_dim=384, depth=[False] * 16,
                 num_heads=12, mlp_ratio=3, qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=partial(nn.LayerNorm, eps=1e-6), group = False,
                 re_atten=True, cos_reg = False, use_cnn_embed=False, apply_transform=[False] * 0 + [True] * 16, 
                 transform_scale= False, scale_adjustment=0.):
        super().__init__()
