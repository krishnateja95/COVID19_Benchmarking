...
__all__ = ['FocalTransformer_Small']
...
def get_relative_position_index(q_windows, k_windows):
    ...
    relative_coords[:, :, 0] += k_windows[0] - 1  
    ...
    def __init__(self, dim, expand_size, window_size, focal_window, focal_level, num_heads, 
        ...
        if self.expand_size > 0 and focal_level > 0:
            ...
            self.relative_position_bias_table_to_neighbors = nn.Parameter(
                torch.zeros(1, num_heads, self.window_size[0] * self.window_size[1], self.window_size_of_key))  
...
class FocalTransformer_Small(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=3, embed_dim=96, depths=[2,2,18,2], num_heads=[3, 6, 12, 24],
                window_size=7, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.2,
                norm_layer=nn.LayerNorm, ape=False, patch_norm=True, use_checkpoint=False, use_shift=False, focal_stages=[0, 1, 2, 3], 
                focal_levels=[2,2,2,2], focal_windows=[7, 5, 3, 1], focal_pool="fc", expand_stages=[0, 1, 2, 3], expand_sizes=[3, 3, 3, 3],
                expand_layer="all", use_conv_embed=True, use_layerscale=False, layerscale_value=1e-4, use_pre_norm=False,):
        super().__init__()
