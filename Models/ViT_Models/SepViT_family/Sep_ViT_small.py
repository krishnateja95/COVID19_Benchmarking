...
class SepViTBlock(TimmBlock):
    def __init__(self, dim,
                 ...
                 ws=7,
                 stage=0):
        super(SepViTBlock, self).__init__(dim,
                                          num_heads,
                                          mlp_ratio,
                                          qkv_bias,
                                          qk_scale,
...
class Sep_ViT_Small(nn.Module):
    def __init__(self, img_size=224, patch_size=7, in_chans=3, num_classes=3, embed_dims=[96, 192, 384, 768],
                 num_heads=[3, 6, 12, 24], mlp_ratios=[4, 4, 4, 4], qkv_bias=True, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 depths=[1, 2, 14, 2], block_cls=SepViTBlock, ws=[[7], [7, 14], [7, 14]*7, [7, 7]]):
        super().__init__()
        ...
        for i in range(len(depths)):
            if i == 0:
                self.patch_embeds.append(PatchEmbed(img_size=img_size, patch_size=patch_size, stride=4, 
                                                    in_chans=in_chans, embed_dim=embed_dims[i]))
            else: 
                self.patch_embeds.append(PatchEmbed(img_size=img_size // 4 // 2 ** (i - 1), patch_size=3, 
        ...
        for k in range(len(depths)):
            _block = nn.ModuleList([block_cls(
                ...
                ws=ws[k][i], stage=k) for i in range(depths[k])])
